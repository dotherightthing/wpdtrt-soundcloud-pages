...
if ( !function_exists( 'wpdtrt_soundcloud_pages_post_type_create' ) ) {
    ...
    function wpdtrt_soundcloud_pages_post_type_create() {
        ...
        register_post_type( 'soundcloud_pages',
            array(
                'labels' =>                 $labels,
            ...
            'taxonomies' =>                 array( 'playlisttype' ),
            'has_archive' =>                false,
            'rewrite' =>                    array(
                'slug' => 'music/%playlisttype%',
                'with_front' => false
            ),
...
if ( !function_exists( 'wpdtrt_soundcloud_pages_post_type_permalink_replacement' ) ) {
     ...
     */
    function wpdtrt_soundcloud_pages_post_type_permalink_replacement( $post_link, $id = 0 ){

        $post = get_post($id);

        if ( is_object( $post ) ){

            $terms = wp_get_object_terms( $post->ID, 'playlisttype' );

            if ( $terms ) {
                return str_replace( '%playlisttype%' , $terms[0]->slug , $post_link );
            }
        }

        return $post_link;
    }
    add_filter( 'post_type_link', 'wpdtrt_soundcloud_pages_post_type_permalink_replacement', 10, 2 );
