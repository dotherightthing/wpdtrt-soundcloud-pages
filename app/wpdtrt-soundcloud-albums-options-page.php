...
if ( !function_exists( 'wpdtrt_soundcloud_albums_menu' ) ) {
   ...
   */
  function wpdtrt_soundcloud_albums_menu() {
    ...
    add_options_page(
      ...
      'manage_options',
      'soundcloud-albums',
      'wpdtrt_soundcloud_albums_options_page'
    );
  }

  add_action('admin_menu', 'wpdtrt_soundcloud_albums_menu');

}

if ( !function_exists( 'wpdtrt_soundcloud_albums_options_page' ) ) {

  /**
   * Render the appropriate UI on Settings > SoundCloud Albums
   *
   ...
   *       a. The user's selection: 'wpdtrt_soundcloud_albums_username',
   *          so that the dataset can be regenerated after a specified amount of time
   *       b. The data set generated by the API: 'wpdtrt_soundcloud_albums_data'
   ...
   * @since       0.1.0
   */
  function wpdtrt_soundcloud_albums_options_page() {
    ...
    if ( isset( $_POST['wpdtrt_soundcloud_albums_form_submitted'] ) ) {

      // check that the form submission was legitimate
      $hidden_field = esc_html( $_POST['wpdtrt_soundcloud_albums_form_submitted'] );

      if ( $hidden_field === 'Y' ) {

        // 1. get user preferences from form submission
        $wpdtrt_soundcloud_albums_username = esc_html( $_POST['wpdtrt_soundcloud_albums_username'] );
        $wpdtrt_soundcloud_albums_clientid = esc_html( $_POST['wpdtrt_soundcloud_albums_clientid'] );

        // 2. send user preferences to API to get results
        $wpdtrt_soundcloud_albums_data = wpdtrt_soundcloud_albums_get_data(
          $wpdtrt_soundcloud_albums_username,
          $wpdtrt_soundcloud_albums_clientid
        );

        // 3a. store user preferences in options object
        $wpdtrt_soundcloud_albums_options['wpdtrt_soundcloud_albums_username'] = $wpdtrt_soundcloud_albums_username;
        $wpdtrt_soundcloud_albums_options['wpdtrt_soundcloud_albums_clientid'] = $wpdtrt_soundcloud_albums_clientid;

        // 3b. store API results in options object
        $wpdtrt_soundcloud_albums_options['wpdtrt_soundcloud_albums_data'] = $wpdtrt_soundcloud_albums_data;

        // 3c. store timestamp in options object
        $wpdtrt_soundcloud_albums_options['last_updated'] = time(); // UNIX timestamp for the current time
         ...
         */
        update_option( 'wpdtrt_soundcloud_albums', $wpdtrt_soundcloud_albums_options, null );
      }
    }
    ...
    $wpdtrt_soundcloud_albums_albums_table = new WpDTRT_SoundCloud_Albums_List_Table($wpdtrt_soundcloud_albums_data);
    $wpdtrt_soundcloud_albums_albums_table->prepare_items();

    /**
     * When the user clicks the Regenerate Pages button
     *  generate new pages or update existing pages
     */
    if ( isset( $_GET['wpdtrt_soundcloud_albums_regenerate'] ) ) {
      $regenerate = esc_html( $_GET['wpdtrt_soundcloud_albums_regenerate'] );

      if ( $regenerate === 'Y' ) {
        wpdtrt_soundcloud_albums_create_posts();
      }
     ...
     */
    require_once(WPDTRT_SOUNDCLOUD_ALBUMS_PATH . 'views/admin/partials/wpdtrt-soundcloud-albums-options-page.php');
  }
